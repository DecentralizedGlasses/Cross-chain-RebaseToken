...
output=$(forge script ./script/Deployer.s.sol:TokenAndPoolDeployer --rpc-url ${BASE_SEPOLIA_RPC_URL} --account sepolia --broadcast)
...
output=$(forge script ./script/Deployer.s.sol:TokenAndPoolDeployer --rpc-url ${SEPOLIA_RPC_URL} --account sepolia --broadcast)
echo "Contracts deployed and permission set on Sepolia"
...
VAULT_ADDRESS=$(forge script ./script/Deployer.s.sol:VaultDeployer --rpc-url ${SEPOLIA_RPC_URL} --account sepolia --broadcast --sig "run(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} | grep 'vault: contract Vault' | awk '{print $NF}')
echo "Vault address: $VAULT_ADDRESS"

Configure the pool on Sepolia
echo "Configuring the pool on Sepolia..."
forge script ./script/ConfigurePool.s.sol:ConfigurePoolScript --rpc-url ${SEPOLIA_RPC_URL} --account sepolia --broadcast --sig "run(address,uint64,address,address,bool,uint128,uint128,bool,uint128,uint128)" ${SEPOLIA_POOL_ADDRESS} ${BASE_SEPOLIA_CHAIN_SELECTOR} ${BASE_SEPOLIA_POOL_ADDRESS} ${BASE_SEPOLIA_REBASE_TOKEN_ADDRESS} false 0 0 false 0 0
...
forge script ./script/ConfigurePool.s.sol:ConfigurePoolScript --rpc-url ${BASE_SEPOLIA_RPC_URL} --account sepolia --broadcast --sig "run(address,uint64,address,address,bool,uint128,uint128,bool,uint128,uint128)" ${BASE_SEPOLIA_POOL_ADDRESS} ${SEPOLIA_CHAIN_SELECTOR} ${SEPOLIA_POOL_ADDRESS} ${SEPOLIA_REBASE_TOKEN_ADDRESS} false 0 0 false 0 0

# Bridge the funds using the script to Base Sepolia 
echo "Bridging the funds using the script to Base Sepolia..."
SEPOLIA_BALANCE_BEFORE=$(cast balance $(cast wallet address --account sepolia) --erc20 ${SEPOLIA_REBASE_TOKEN_ADDRESS} --rpc-url ${SEPOLIA_RPC_URL})
echo "Sepolia balance before bridging: $SEPOLIA_BALANCE_BEFORE"
forge script ./script/BridgeTokens.s.sol:BridgeTokensScript --rpc-url ${SEPOLIA_RPC_URL} --account sepolia --broadcast --sig "run(address,uint64,address,uint256,address,address)" $(cast wallet address --account sepolia) ${BASE_SEPOLIA_CHAIN_SELECTOR} ${SEPOLIA_REBASE_TOKEN_ADDRESS} ${AMOUNT} ${SEPOLIA_LINK_ADDRESS} ${SEPOLIA_ROUTER}
echo "Funds bridged to Base Sepolia"
SEPOLIA_BALANCE_AFTER=$(cast balance $(cast wallet address --account sepolia) --erc20 ${SEPOLIA_REBASE_TOKEN_ADDRESS} --rpc-url ${SEPOLIA_RPC_URL})
